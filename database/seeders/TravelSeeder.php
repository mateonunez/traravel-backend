...
class TravelSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        $payloadUAE = [
            ...
            'name' => 'United Emirates: from Dubai to Abu Dhabi',
        ...
        $jodanTravel = Travel::create($payloadJordan);
        $icelandTravel = Travel::create($payloadIceland);
        $uaeTravel = Travel::create($payloadUAE);

        for ($i = 0; $i < count($moods); $i++) {
            $jodanTravel->moods()->attach(
                $moods[$i],
                ['rating' => rand(1, 100)]
            );

            $icelandTravel->moods()->attach(
                $moods[$i],
                ['rating' => rand(1, 100)]
            );

            $uaeTravel->moods()->attach(
                $moods[$i],
                ['rating' => rand(1, 100)]
            );
        }
